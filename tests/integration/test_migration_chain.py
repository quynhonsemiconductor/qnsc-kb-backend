...
def test_migrations_have_one_current_head():
    ...
    # means one was chained off the wrong parent. Bumped for 20260913_82, the review-queue
    # search indexes, chained off 20260912_81 (article_chunks.content_hash) -- itself the
    # last of five chained off 20260911_76 (the tenant knowledge graph): the pgvector
    # column realigned to 1024 for multilingual-e5-large-instruct (77), a re-index after
    # adding contextual chunk headers (78), conflict_records.contradiction_type (79), and
    # approval_rules.risk_tiers/version plus the approval_rule_versions history table (80).
    assert script.get_heads() == ["20260913_82"]
