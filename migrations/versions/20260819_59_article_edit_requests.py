...
def upgrade() -> None:
    ...
    # table already exists on a database created from empty.
    if not inspect(op.get_bind()).has_table("article_edit_requests"):
        op.create_table(
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_article_edit_requests_company_domain ON article_edit_requests (company_domain)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_article_edit_requests_company_status ON article_edit_requests (company_domain, status)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_article_edit_requests_article ON article_edit_requests (article_id)")
