...
def _reject_cross_site_auth_request(request: Request) -> None:
    ...
    if origin and origin not in settings.cors_origin_list:
        raise HTTPException(
            status_code=403, detail="Cross-site authentication requests are not allowed"
        )
...
def _user_response(user: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "active": user.active,
        "roles": [
            {"id": role.id, "name": role.name, "company_domain": role.company_domain}
...
async def _resolve_owned_departments(
    db: AsyncSession, company_domain: str, department_ids: list[uuid.UUID] | None
) -> list[Department]:
    ...
    if len(departments) != len(unique_ids):
        raise HTTPException(
            status_code=422,
            detail="Every owned department must be active and belong to the user's company",
        )
    return departments
...
async def _resolve_membership_departments(
    ...
    if department_ids is None:
        department = await resolve_active_department(
            db, company_domain, primary_name, required=False
        )
    ...
    if len(departments) != len(unique_ids):
        raise HTTPException(
            status_code=422,
            detail="Every department membership must be active and belong to the user's company",
        )
...
async def _ensure_departments_have_single_owner(
    ...
    if conflicts:
        raise HTTPException(
            status_code=409,
            detail=f"Department already has an owner: {', '.join(sorted(set(conflicts)))}",
        )


def _ensure_owned_departments_are_memberships(
    memberships: list[Department], owned: list[Department]
) -> None:
    ...
    if any(department.id not in membership_ids for department in owned):
        raise HTTPException(
            status_code=422,
            detail="A user must be a member of every department they own",
        )
...
def _apply_department_ownership(user: Any, departments: list[Department]) -> None:
    ...
    for department_id, department in desired_by_id.items():
        if department_id not in existing_ids:
            user.department_ownerships.append(
                DepartmentManager(department=department, user=user, active=True)
            )


def _is_global_user_manager(user: Any) -> bool:
    return AuthorizationService.has_permission(
        user, "user.manage", requested_scope="global"
    )


def _is_global_role_manager(user: Any) -> bool:
    return AuthorizationService.has_permission(
        user, "role.manage", requested_scope="global"
    )
...
async def _system_role(db: AsyncSession, name: str, company_domain: str | None) -> Role:
    role = (
        await db.execute(
            select(Role).where(Role.name == name, Role.company_domain == company_domain)
        )
    ).scalar_one_or_none()
    if role is None:
        raise HTTPException(
            status_code=422, detail=f"The requested system role is unavailable: {name}"
        )
    return role
...
def _validate_role_assignment_authority(current_user: Any, roles: list[Role]) -> None:
    ...
    if any(
        role.name.casefold() == LEGACY_DEPARTMENT_OWNER_ROLE.casefold()
        for role in roles
    ):
        raise HTTPException(
            status_code=422,
            detail="Department ownership is managed separately from roles",
        )
    if _is_global_user_manager(current_user):
        return
    if any(
        role.company_domain is None or role.name in {"Admin", "CEO"} for role in roles
    ):
        raise HTTPException(
            status_code=403,
    ...
    effective = {
        item["key"]: item["scope"]
        for item in AuthorizationService.get_effective_permissions(current_user)
    }
    for role in roles:
        for assignment in role.permissions:
            if assignment.permission and rank.get(assignment.scope, 0) > rank.get(
                effective.get(assignment.permission.key, ""), 0
            ):
                raise HTTPException(
                    status_code=403,
...
async def _store_refresh_session(db: AsyncSession, user: Any, token: str) -> None:
    ...
    expires_at = datetime.fromtimestamp(int(payload["exp"]), tz=timezone.utc).replace(
        tzinfo=None
    )
    db.add(
        RefreshSession(
...
async def list_users(
    ...
    users = await UserRepository(db).list_users(
        offset=offset, limit=limit, viewer=current_user
    )
...
async def list_departments(
    ...
    can_manage = AuthorizationService.has_permission(
        current_user, "user.manage", requested_scope="company"
    )
    stmt = (
        select(Department)
...
async def create_department(
    ...
    # Department creation is always scoped to the one company represented by
    # the signed-in user.  Ignore a legacy company_domain request field rather
    # than allowing a global manager to create a second company namespace.
    ...
    await AuditRepository(db).record(
        current_user.id, "department_create", "department", str(item.id)
    )
    return {
        "id": item.id,
        "name": item.name,
...
async def _department_for_management(
    db: AsyncSession, department_id: uuid.UUID, current_user: Any
) -> Department:
...
async def update_department(
    ...
    if payload.name is not None:
        ...
        if name is None:
            raise HTTPException(
                status_code=422, detail="Department name cannot be blank"
            )
        duplicate = (
            await db.execute(
                select(Department).where(
        ...
        await db.execute(
            update(User)
            .where(User.company_domain == item.company_domain, User.dept == old_name)
            .values(dept=name)
        )
        await db.execute(
            update(Article)
            .where(
                Article.company_domain == item.company_domain, Article.dept == old_name
            )
            .values(dept=name)
        )
        await db.execute(
        ...
        await db.execute(
            update(PendingDraft)
            .where(
                PendingDraft.company_domain == item.company_domain,
                PendingDraft.dept == old_name,
            )
            .values(dept=name)
        )
        await db.execute(
            update(Gap)
            .where(Gap.company_domain == item.company_domain, Gap.dept == old_name)
            .values(dept=name)
        )
        await db.execute(
            update(FeatureFlag)
    ...
    await db.refresh(item)
    await AuditRepository(db).record(
        current_user.id, "department_update", "department", str(item.id)
    )
    return {
        "id": item.id,
        "name": item.name,
...
async def delete_department(
        ...
        "users": int(
            (
                await db.execute(
                    ...
                    .select_from(User)
                    .where(
                        User.company_domain == item.company_domain,
                        User.dept == item.name,
                    )
                )
            ).scalar_one()
        ),
        "user memberships": int(
            (
                await db.execute(
                    select(func.count())
                    .select_from(user_departments)
                    .where(user_departments.c.department_id == item.id)
                )
            ).scalar_one()
        ),
        "articles": int(
            (
                await db.execute(
                    ...
                    .where(
                        Article.company_domain == item.company_domain,
                        Article.dept == item.name,
                    )
                )
            ).scalar_one()
        ),
        "article associations": int(
            (
                await db.execute(
                    select(func.count())
                    .select_from(article_departments)
                    .where(article_departments.c.department_id == item.id)
                )
            ).scalar_one()
        ),
        "pending drafts": int(
        ...
        "gaps": int(
            (
                await db.execute(
                    ...
                    .select_from(Gap)
                    .where(
                        Gap.company_domain == item.company_domain, Gap.dept == item.name
                    )
                )
            ).scalar_one()
        ),
        "owners": int(
            (
                await db.execute(
                    ...
                    .where(DepartmentManager.department_id == item.id)
                )
            ).scalar_one()
        ),
        "feature flags": int(
            (
                await db.execute(
                    ...
                    .where(FeatureFlag.department == item.name)
                )
            ).scalar_one()
        ),
    }
    ...
    await db.commit()
    await AuditRepository(db).record(
        current_user.id, "department_delete", "department", str(department_id)
    )
...
async def create_managed_user(
    ...
    if not can_manage_globally and user_in.role in {"Admin", "CEO"}:
        raise HTTPException(
            status_code=403,
            detail="Only global user managers can create executive or Admin accounts",
        )
    memberships = await _resolve_membership_departments(
        db, domain, user_in.department_ids, user_in.dept
    )
    department = (
        await resolve_active_department(db, domain, user_in.dept, required=False)
        if user_in.dept
        else (memberships[0] if memberships else None)
    )
    if (
        department
        and memberships
        and department.id not in {item.id for item in memberships}
    ):
        raise HTTPException(
            status_code=422,
            detail="The primary department must be one of the selected memberships",
        )
    owned_departments = await _resolve_owned_departments(
        db, domain, user_in.owned_department_ids
    )
    ...
    if user_in.role_ids is not None:
        ...
        if len(requested_roles) != len(set(user_in.role_ids)) or any(
            role.company_domain not in (None, domain) for role in requested_roles
        ):
            raise HTTPException(status_code=403, detail="Invalid role assignment")
        if any(not role.active for role in requested_roles):
            raise HTTPException(
                status_code=422, detail="Inactive roles cannot be assigned"
            )
    ...
    user = await AuthService(UserRepository(db)).register_user(
        ...
        name=user_in.name,
        password=password,
        dept=department.name if department else None,
        role=user_in.role,
    ...
    if requested_roles is not None:
        ...
        user.role = next(
            (
                ...
                if role.name in MANAGED_PRIMARY_ROLES
            ),
            requested_roles[0].name,
        )
    else:
        user.role = user.role if user.role in MANAGED_PRIMARY_ROLES else "Staff"
    owned_departments = await _resolve_owned_departments(
        db, user.company_domain, user_in.owned_department_ids
    )
    ...
    user = await UserRepository(db).get_by_id(user.id)
    await AuditRepository(db).record(
        current_user.id, "user_create", "user", str(user.id)
    )
...
async def update_managed_user(
    ...
    if (
        not AuthorizationService.has_permission(
            current_user, "user.manage", requested_scope="global"
        )
        and user.company_domain != current_user.company_domain
    ):
    ...
    if "dept" in user_in.model_fields_set:
        ...
        memberships = await _resolve_membership_departments(
            db, user.company_domain, user_in.department_ids, user_in.dept
        )
        primary = (
            await resolve_active_department(
                db, user.company_domain, user_in.dept, required=False
            )
            if user_in.dept
            else None
        )
        if (
            primary
            and memberships
            and primary.id not in {item.id for item in memberships}
        ):
            raise HTTPException(
                status_code=422,
                detail="The primary department must be one of the selected memberships",
        ...
        user.departments = memberships
        user.dept = (
            primary.name if primary else (memberships[0].name if memberships else None)
        )
    elif user_in.department_ids is not None:
        memberships_changed = True
        memberships = await _resolve_membership_departments(
            db, user.company_domain, user_in.department_ids, user.dept
        )
        user.departments = memberships
        membership_names = {item.name for item in memberships}
        user.dept = (
            user.dept
            if user.dept in membership_names
            else (memberships[0].name if memberships else None)
        )
    if user_in.role is not None:
        ...
        if not can_manage_globally and user_in.role in {"Admin", "CEO"}:
            raise HTTPException(
                status_code=403, detail="Company managers cannot assign global roles"
            )
    ...
    if user_in.role_ids is not None:
        roles = (
            (
                await db.execute(
                    ...
                    .where(
                        Role.id.in_(user_in.role_ids),
                        (Role.company_domain == user.company_domain)
                        | Role.company_domain.is_(None),
                    )
                    .options(
                        selectinload(Role.permissions).selectinload(
                            RolePermission.permission
        ...
        if any(
            role.company_domain not in (None, user.company_domain) for role in roles
        ):
            raise HTTPException(
                status_code=403,
                detail="A user can only receive roles from their company",
            )
        ...
        if (
            ...
            and can_manage_globally
            and not any(
                role.name == "Admin" and role.company_domain is None for role in roles
            )
        ):
            raise HTTPException(
                status_code=400, detail="You cannot remove your own global Admin role"
            )
        user.roles = list(roles)
        user.role = next(
            (role.name for role in roles if role.name in MANAGED_PRIMARY_ROLES),
            roles[0].name,
        )
    if user_in.owned_department_ids is not None:
        owned_departments = await _resolve_owned_departments(
    ...
    if memberships_changed:
        if user_in.owned_department_ids is not None:
            ...
            retained_owned = [
                ...
                if department.id in membership_ids
            ]
    ...
    if user_in.email is not None:
        ...
        if (
            not can_manage_globally
            and new_email.rsplit("@", 1)[-1] != current_user.company_domain
        ):
            raise HTTPException(
                status_code=403,
                detail="Employee email must remain in the company domain",
            )
    ...
    await AuditRepository(db).record(
        current_user.id, "user_update", "user", str(user.id)
    )
...
async def deactivate_user(
    ...
    updated = await repo.update(user)
    await AuditRepository(db).record(
        current_user.id, "user_deactivate", "user", str(user.id)
    )
    return _user_response(updated)
...
async def assign_company_ceo(
    ...
    if not _is_global_user_manager(current_user):
        raise HTTPException(
            status_code=403, detail="Only global user managers can assign a company CEO"
        )
    repo = UserRepository(db)
    users = await repo.list_users(viewer=current_user)
    company_users = [
        user for user in users if user.company_domain == company_domain.lower()
    ]
    target = next((user for user in company_users if user.id == user_id), None)
    if not target:
        raise HTTPException(
            status_code=404, detail="Employee not found in this company"
        )
    if not target.active:
        raise HTTPException(
            status_code=422, detail="A deactivated employee cannot be assigned as CEO"
        )
    ...
    for user in company_users:
        if (
            user.role == "CEO" or any(role.name == "CEO" for role in user.roles)
        ) and user.id != target.id:
            user.role = "Staff"
            user.roles = [role for role in user.roles if role.name != "CEO"] or [
                staff_role
            ]
            await repo.update(user)
            await AuditRepository(db).record(
                current_user.id, "ceo_change", "user", str(user.id)
            )
    target.role = "CEO"
    target.roles = [role for role in target.roles if role.name != "CEO"] + [ceo_role]
    target = await repo.update(target)
    await AuditRepository(db).record(
        current_user.id, "ceo_change", "user", str(target.id)
    )
...
async def list_permissions(
    ...
    if not (
        ...
        or AuthorizationService.has_permission(
            current_user, "role.manage", requested_scope="company"
        )
    ):
        raise HTTPException(
            status_code=403, detail="Missing permission to view the permission catalog"
        )
    permissions = (
        (await db.execute(select(Permission).order_by(Permission.key))).scalars().all()
    )
    return [
        {
            "id": item.id,
...
async def list_roles(
    ...
    if not (
        ...
        raise HTTPException(
            status_code=403, detail="Missing permission to view assignable roles"
        )
    stmt = (
        select(Role)
        .where(func.lower(Role.name) != LEGACY_DEPARTMENT_OWNER_ROLE.casefold())
        .options(selectinload(Role.permissions).selectinload(RolePermission.permission))
        .order_by(Role.company_domain, Role.name)
    )
    if not (
        AuthorizationService.has_permission(
        ...
        stmt = stmt.where(
            (Role.company_domain == current_user.company_domain)
            | (Role.company_domain.is_(None))
        )
    ...
    return [
        {
            ...
            "system": role.system,
            "permissions": [
                {"permission_key": item.permission.key, "scope": item.scope}
...
async def create_role(
    payload: RoleCreate,
    current_user: Any = Depends(require_permission("role.manage")),
    db: AsyncSession = Depends(get_db),
) -> Any:
    can_manage_globally = AuthorizationService.has_permission(
        current_user, "role.manage", requested_scope="global"
    ...
    company_domain = (
        payload.company_domain.lower()
        if payload.company_domain
        else (None if can_manage_globally else current_user.company_domain)
    )
    ...
    await AuditRepository(db).record(
        current_user.id, "role_create", "role", str(role.id)
    )
    return {
        "id": role.id,
        "name": role.name,
...
async def update_role(
    ...
    if (role.company_domain is None and not can_manage_globally) or (
        role.company_domain is not None
        and role.company_domain != current_user.company_domain
        and not can_manage_globally
    ):
        raise HTTPException(
            status_code=403, detail="Role is outside your management scope"
        )
    if role.system and payload.name and payload.name != role.name:
        raise HTTPException(
            status_code=400, detail="System role names cannot be changed"
        )
    if payload.name is not None:
        role_name = payload.name.strip()
        if not role_name:
    ...
    if payload.active is not None and not role.system:
        role.active = payload.active
    await db.commit()
    await AuditRepository(db).record(
        current_user.id, "role_update", "role", str(role.id)
    )
    return {
        "id": role.id,
        "name": role.name,
        ...
        "system": role.system,
        "permissions": [
            {"key": item.permission.key, "scope": item.scope}
            for item in role.permissions
            if item.permission
        ],
    }
...
async def delete_role(
    ...
    if not can_manage_globally:
        role_stmt = role_stmt.where(Role.company_domain == current_user.company_domain)
    role = (
        await db.execute(role_stmt.options(selectinload(Role.users)))
    ).scalar_one_or_none()
    if not role:
        raise HTTPException(status_code=404, detail="Role not found")
    if role.system:
    ...
    if role.users:
        raise HTTPException(
            status_code=409, detail="Remove users from this role before deleting it"
        )
    await db.delete(role)
    await db.commit()
    await AuditRepository(db).record(
        current_user.id, "role_delete", "role", str(role.id)
    )
...
async def replace_role_permissions(
    role_id: uuid.UUID,
    payload: list[RolePermissionInput] = Body(..., max_length=100),
    current_user: Any = Depends(require_permission("permission.manage")),
    db: AsyncSession = Depends(get_db),
) -> Any:
    can_manage_globally = AuthorizationService.has_permission(
        current_user, "permission.manage", requested_scope="global"
    )
    role_stmt = select(Role).where(Role.id == role_id)
    if not can_manage_globally:
        role_stmt = role_stmt.where(Role.company_domain == current_user.company_domain)
    ...
    if role.system:
        raise HTTPException(
            status_code=400, detail="System role permissions cannot be changed"
        )
    if (role.company_domain is None and not can_manage_globally) or (
        role.company_domain is not None
        and role.company_domain != current_user.company_domain
    ...
    if len(permission_keys) != len(set(permission_keys)):
        raise HTTPException(
            status_code=422, detail="A permission can only be assigned once per role"
        )
    if (role.company_domain is not None or not can_manage_globally) and any(
        item.scope == "global" for item in payload
    ):
        raise HTTPException(
            status_code=403,
            detail="Only global permission managers can grant global scopes to global roles",
        )
    ...
    if not can_manage_globally and any(
        rank[item.scope] > rank.get(effective_scopes.get(item.permission_key, ""), 0)
        for item in payload
    ):
        raise HTTPException(
            status_code=403,
            detail="You cannot delegate permissions broader than your own",
        )
    ...
    for item in payload:
        db.add(
            RolePermission(
                role_id=role.id,
                permission_id=by_key[item.permission_key].id,
                scope=item.scope,
            )
        )
    await db.commit()
    await AuditRepository(db).record(
        current_user.id, "role_permissions_update", "role", str(role.id)
    )
...
async def replace_user_roles(
    ...
    if len(roles) != len(set(role_ids)) or any(
        role.company_domain not in (None, user.company_domain) for role in roles
    ):
        raise HTTPException(status_code=403, detail="Invalid role assignment")
    if any(not role.active for role in roles):
    ...
    if (
        ...
        and AuthorizationService.has_permission(
            current_user, "user.manage", requested_scope="global"
        )
    ):
        raise HTTPException(
    ...
    user = await UserRepository(db).get_by_id(user.id)
    await AuditRepository(db).record(
        current_user.id, "user_roles_update", "user", str(user.id)
    )
    return _user_response(user)
...
async def login(
    ...
    account_allowed, account_retry = await auth_rate_limiter.allow(
        f"account:{username}"
    )
...
async def refresh_token(
    ...
    _reject_cross_site_auth_request(request)
    refresh_value = (
        req.refresh_token if req and req.refresh_token else refresh_token_cookie
    )
    ...
    if session is None:
        raise HTTPException(
            status_code=401, detail="Refresh token has been revoked or expired"
        )
    ...
    if (
        ...
        or int(payload.get("av", 0)) != user.auth_version
    ):
...
async def entra_login() -> dict[str, str]:
    ...
    if not entra_auth.configured():
        raise HTTPException(
            status_code=503, detail="Microsoft Entra login is not configured"
        )
    ...
    return {
        "authorization_url": entra_auth.authorization_url(state, nonce),
...
async def entra_callback(
    ...
    try:
        ...
        claims = await entra_auth.verify_id_token(
            str(tokens.get("id_token") or ""), str(state_claims["nonce"])
        )
    except (JWTError, ValueError, KeyError) as exc:
        raise HTTPException(
            status_code=401, detail="Microsoft sign-in could not be verified"
        ) from exc
    ...
    user = (
        await UserRepository(db).get_by_id(identity.user_id)
        if identity
    ...
    if not user.active:
        raise HTTPException(
            status_code=403,
            detail="Your Microsoft account is not linked to an active internal KB account",
        )
    if user.email.lower() != email:
        raise HTTPException(
            status_code=403,
            detail="The Microsoft account email does not match the linked internal account",
        )
    if identity is None:
        db.add(
            ExternalIdentity(
        ...
        await AuditRepository(db).record(
            user.id, "entra_identity_link", "user", str(user.id), outcome="success"
        )
    if not user.roles:
        await bootstrap_rbac(db)
        user = await UserRepository(db).get_by_id(user.id)
